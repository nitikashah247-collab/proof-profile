      ...
      >
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <span className="font-metrics text-2xl font-bold text-foreground">
              {study.keyMetric}
            </span>
            <span className="text-xs font-medium uppercase tracking-wider text-primary/70 bg-primary/10 px-2.5 py-1 rounded-full">
              {study.company}
            </span>
          </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {study.artifacts.map((artifact, i) => (
                        <div
                          key={i}
                          className="rounded-xl border border-border overflow-hidden bg-muted/30 cursor-pointer hover:border-primary/30 transition-colors"
                          onClick={() => {
                        ...
                        >
                          {isImageType(artifact.type) ? (
                            ...
                            <div className="flex items-center gap-3 p-4">
                              ...
                              <span className="text-sm text-foreground hover:underline">{artifact.caption}</span>
                            </div>
                          )}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {study.images.map((img, i) => (
                        <div key={i} className="aspect-video rounded-xl bg-muted border border-border flex items-center justify-center text-muted-foreground text-xs">
                          Screenshot {i + 1}
                        </div>
                      ))}
                    </div>
